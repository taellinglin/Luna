...
class Blockchain:
    ...
    def save_chain(self):
        ...
        try:
            ...
            print(
                f"💾 Attempting to save {len(chain_data)} blocks to {self.chain_file}"
            )
    ...
    def start_peer_discovery(self):
        ...
        if not self.peer_discovery_running:
            discovery_thread = threading.Thread(
                target=self.peer_discovery_loop, daemon=True
            )
            ...
            if data is not None:
                if resource_type == "blockchain":
                    ...
                    if self.validate_chain(new_chain):
                        ...
                        if hasattr(self, "save_chain") and callable(
                            getattr(self, "save_chain")
                        ):
                            self.save_chain()
                        else:
                        ...
                        print(
                            f"✅ Downloaded and validated blockchain with {len(data)} blocks"
                        )
                        return True
                    else:
                    ...
                    if isinstance(data, list):
                        ...
                        if "transactions" in data:
                            ...
                            self.save_mempool()
                            print(
                                f"✅ Downloaded mempool with {len(data['transactions'])} transactions"
                            )
                            return True
                        else:
                            print(
                                f"❌ Mempool data dict doesn't contain 'transactions' key: {data.keys()}"
                            )
                            return False
                    else:
    ...
    def download_blockchain(self, peer_address: str = None):
        ...
        for peer in peers_to_try:
            try:
                ...
                response = self.send_to_peer(
                    peer, {"action": "get_blockchain"}, timeout=30
                )
                ...
                if response.get("status") == "success":
                    ...
                    print(
                        f"🔍 Received {len(chain_data)} blocks, I have {len(self.chain)} blocks"
                    )

                    if len(chain_data) > len(self.chain):
                        print(
                            f"✅ Downloaded blockchain with {total_blocks} blocks from {peer}"
                        )
                        ...
                        print(
                            "ℹ️  Peer has same or shorter chain, keeping current blockchain"
                        )
                else:
        ...
        if peers_to_try:
            ...
            for peer in peers_to_try:
                try:
                    ...
                    response = self.send_to_peer(
                        peer, {"action": "get_pending_transactions"}
                    )
                    ...
                    if response and response.get("status") == "success":
                        ...
                        for tx in transactions:
                            if not any(
                                t.get("signature") == tx.get("signature")
                        ...
                        print(
                            f"✅ Added {new_transactions} new transactions from {peer}'s mempool"
                        )
                        return True
                    else:
            ...
            if response_text:
                try:
                    ...
                    if new_transactions > 0:
                        print(
                            f"✅ Added {new_transactions} new transactions from web mempool"
                        )
                    else:
    ...
    def increment_user_limit(self, user_id, denomination):
        ...
        self.user_limits[user_id][denomination] = (
            self.user_limits[user_id].get(denomination, 0) + 1
        )
    ...
    def add_transaction(self, transaction: Dict):
        ...
        if self.is_transaction_mined(transaction):
            print(
                f"⏭️  Transaction already mined, skipping: {transaction.get('signature', 'unknown')[:16]}..."
            )
            return False
        ...
        requirements = self.difficulty_requirements.get(
            denomination, {"difficulty": 0, "min_time": 0.1, "max_attempts": 10}
        )
        ...
        tx_data = {"action": "new_transaction", "transaction": transaction}
        ...
        for block in self.chain:
            for tx in block.transactions:
                ...
                if (
                    ...
                    and transaction.get("type") == "GTX_Genesis"
                    and tx.get("serial_number") == transaction.get("serial_number")
                ):
                    return True
        return False
    ...
    def determine_bill_denomination(self, block_hash):
        ...
        available_bills = self.difficulty_denominations.get(
    ...
    def mine_pending_transactions(self, mining_reward_address: str):
        ...
        print(
            f"📦 Preparing block with {len(self.pending_transactions)} transactions..."
        )
        ...
        for tx in self.pending_transactions:
            ...
            max_attempts = requirements.get(
                "max_mining_attempts", self.max_mining_attempts
            )
            ...
            if attempts < max_attempts:
                ...
                print(
                    f"⏭️  Skipping transaction {tx.get('signature', 'unknown')[:16]}... - exceeded max attempts"
                )
        ...
        def mining_progress(stats):
            ...
            print(
                f"\r⛏️  Mining: {stats['hashes']:,.0f} hashes | {hash_rate_str} | Current: {stats['current_hash'][: self.difficulty + 2]}...",
                end="",
                flush=True,
            )
        ...
        print(
            f"💵 Available Bills: {', '.join(self.difficulty_denominations[self.difficulty].keys())}"
        )
        print(
            f"📊 Mineable Transactions: {len(mineable_transactions)}/{len(self.pending_transactions)}"
        )
        print("=" * 60)
        ...
        bill_denomination, multiplier, actual_reward = self.determine_bill_denomination(
            block.hash
        )
        ...
        self.pending_transactions = [
            tx
            for tx in self.pending_transactions
            if tx.get("signature") not in mined_signatures
        ]
        self.save_mempool()
            ...
            if chain_data:
                ...
                for block_data in chain_data:
                    block = Block(
                        ...
                        block_data[
                            "transactions"
                        ],  # ✅ Critical: Load ALL transactions
                        block_data.get("nonce", 0),
                    )
                    block.hash = block_data["hash"]
                    block.mining_time = block_data.get("mining_time", 0)
        ...
        try:
            if action == "ping":
                ...
                if self.add_transaction(transaction):
                    ...
                    return {
                        "status": "error",
                        "message": "Failed to add transaction (spam protection or duplicate)",
                    }
                ...
                if estimated_size > MAX_RESPONSE_SIZE:
                    print(
                        f"⚠️  Large blockchain response: {estimated_size / 1024 / 1024:.1f}MB"
                    )
                ...
                if peer_address and self.add_peer(peer_address):
                    return {
                        "status": "success",
                        "message": f"Peer {peer_address} added",
                    }
                else:
                ...
                if block_data:
                    if block_data["index"] > len(self.chain) - 1:
                        ...
                        if (
                            ...
                            and block.previous_hash == self.get_latest_block().hash
                        ):
                    ...
                    return {
                        "status": "success",
                        "message": "Block already exists or invalid",
                    }
                else:
                ...
                if transaction:
                    if not any(
                        t.get("signature") == transaction.get("signature")
                        for t in self.pending_transactions
                    ):
                        self.pending_transactions.append(transaction)
                        self.save_mempool()
                        print("📥 Received new transaction from peer")
                        return {
                            "status": "success",
                            "message": "Transaction added to mempool",
                        }
    ...
    def wallet_server_thread(self):
        ...
        try:
            ...
            print("👛 Node Wallet server listening on port 9335")
                    ...
                    try:
                        ...
                        while len(request_data) < message_length:
                            chunk = client_socket.recv(
                                min(4096, message_length - len(request_data))
                            )
                            if not chunk:
                                break
                            request_data += chunk
