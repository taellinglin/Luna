...
class ClientMiner:
    ...
    def calculate_block_hash(self, block):
        ...
        if transactions:
            ...
            transactions_string = json.dumps(
                sorted_transactions, sort_keys=True, separators=(",", ":")
            )
        else:
            transactions_string = "[]"
    ...
    def submit_block(self, mined_block):
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/block/submit", json=submission_data, timeout=30
            )
    ...
    def start_mining(self):
        ...
        while self.is_mining:
            try:
                ...
                logging.info(
                    f"⛏️ Mining block #{mining_work['index']} with {len(mining_work['transactions'])} transactions"
                )
                ...
                if mined_block:
                    ...
                    if result.get("status") == "success":
                        logging.info(
                            f"✅ Successfully mined block #{mined_block['index']}!"
                        )
