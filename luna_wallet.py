...
class LunaWallet:
        ...
        while True:
            ...
            if confirm:
                confirm_pw = getpass.getpass(
                    color_text("Confirm password: ", COLORS["B"])
                )
    ...
    def first_time_setup(self):
        ...
        password = self.get_password(
            "🔐 Set wallet encryption password: ", confirm=True
        )
    ...
    def get_cached_password(self):
        ...
        if (
            self.password
            and time.time() - self.password_timestamp < self.PASSWORD_TIMEOUT
        ):
    ...
    def save_wallet(self, password=None, background=False):
        ...
        return SecureDataManager.save_encrypted_wallet(
        ...
        while self.scanning:
            try:
                ...
                if self.updates:
                    ...
                    print(
                        color_text(
                            "📊 Blockchain scan completed - balances updated",
                            COLORS["B"],
                        )
                    )
            ...
            try:
                ...
                if self.updates:
                    if self.save_wallet(background=True):
                        ...
                        print(
                            color_text(
                                "⚠️  Updates found but no cached password - run 'sync' to save",
                                COLORS["Y"],
                            )
                        )
    ...
    def scan_blockchain(self):
        ...
        for wallet in self.wallets:
            ...
            if (
                ...
                or len(wallet["transactions"]) != old_tx_count
            ):
    ...
    def scan_wallet_transactions(self, wallet, blockchain):
        ...
        for block in blockchain:
            ...
            for tx in block.get("transactions", []):
                ...
                if (from_addr and from_addr.lower() == address.lower()) or (
                    ...
                    print(
                        color_text(
                            f"📥 Transaction found: {tx.get('amount', 0)} LKC {direction}",
                            COLORS["B"],
                        )
                    )

        # Recalculate balance from all transactions
        wallet["balance"] = self.calculate_balance_from_transactions(
        ...
        for tx in wallet["transactions"]:
            if (
                ...
                and tx.get("hash") in [ptx.get("hash") for ptx in self.pending_txs]
            ):
                wallet["pending_send"] = max(
                    0, wallet["pending_send"] - float(tx.get("amount", 0))
                )
    ...
    def update_pending_transactions(self, blockchain):
        ...
        for pending_tx in self.pending_txs[:]:
            if pending_tx.get("hash") in blockchain_hashes:
                ...
                print(
                    color_text(
                        f"✅ Transaction confirmed: {pending_tx.get('hash', '')[:16]}...",
                        COLORS["G"],
                    )
                )
                ...
                print(
                    color_text(
                        f"❌ Transaction failed: {pending_tx.get('hash', '')[:16]}...",
                        COLORS["R"],
                    )
                )
    ...
    def send(self, to_address, amount, memo=""):
        ...
        if available_balance < amount:
            print(
                color_text(
                    f"❌ Insufficient balance. Available: {available_balance} LKC",
                    COLORS["R"],
                )
            )
            return False
        ...
        try:
            response = requests.post(
                "https://bank.linglin.art/mempool/add", json=tx, timeout=10
            )
    ...
    def verify_password(self, password):
        ...
        try:
            test_data = {"test": "data"}
            encrypted = SecureDataManager.save_encrypted_wallet(
                "test.tmp", test_data, password
            )
            if encrypted:
    ...
    def show_transaction_history(self):
        ...
        for i, tx in enumerate(all_transactions, 1):
            ...
            print(
                f"{i}. {status_color}{status_icon} {type_icon} {amount:.6f} LKC{COLORS['X']}"
            )
...
def main():
    ...
    print(
        color_text(
            "\n💡 Commands: status, send, receive, history, sync, import, export, new, exit",
            COLORS["I"],
        )
    )
