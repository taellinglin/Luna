        ...
        try:
            ...
            for mined_reward in self.config.reward_transactions_mined:
                if (mined_reward.get('hash') == reward_signature or 
                    (mined_reward.get('recipient') == recipient and 
                     mined_reward.get('block_height') == block_height and
                     mined_reward.get('amount') == amount)):
                    return True
                ...
                for tx in transactions:
                    ...
                    if tx_type in ['transfer', 'GTX_Genesis']:
                        ...
                        if not self.is_reward_already_mined(tx):
                            ...
                            print(color_text(f"⏭️  Skipping already mined reward: {tx.get('to')} for {tx.get('amount')} LUN", Colors.YELLOW))
                
                print(color_text(f"📥 Using {len(valid_txs)} regular + {len(reward_txs)} unmined reward transactions", Colors.BLUE))
    ...
    def submit_block(self, block_data):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(color_text(f"❌ HTTP {response.status_code}: {response.text}", COLORS["R"]))
