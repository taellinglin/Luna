...
class Wallet:
    ...
    def create_transfer(self, to_address, amount):
        ...
        with self.lock:
            ...
            transaction["signature"] = hashlib.sha256(transaction_data.encode()).hexdigest()
...
class BlockchainState:
    ...
    def get_next_block_info():
            ...
            if status_response.status_code == 200:
                ...
                if blockchain_response.status_code == 200:
                    ...
                    if actual_blocks_count > 0:
                        ...
                        logger.info(f"📊 Last block hash: {actual_previous_hash[:16]}...")
                    else:
                ...
                logger.info(f"📊 Blockchain state: reported_height={reported_height}, next_index={next_index}")
        ...
        return {'next_index': 0, 'previous_hash': '0' * 64, 'difficulty': MINING_DIFFICULTY, 'reported_height': 0, 'actual_blocks': 0}
...
class SmartMiner:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                if result.get("success"):
                    ...
                    logger.info(f"📤 Transfer submitted: {transfer_tx['from'][:8]} → {transfer_tx['to'][:8]} ({transfer_tx['amount']} LUN)")
                    return True
                else:
                    logger.warning(f"❌ Transfer rejected: {result.get('error', 'Unknown error')}")
            else:
    ...
    def generate_random_transfer(self):
        ...
        transfer_tx = sender.create_transfer(receiver.address, 1.0)  # Fixed small amount for testing
    ...
    def get_mempool_transactions(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"📥 Using {len(valid_txs)}/{len(transactions)} valid mempool transactions")
    ...
    def validate_reward_transaction(self, reward_tx, block_index):
        ...
        if reward_tx.get('block_height') != block_index:
            logger.error(f"❌ Reward block_height mismatch: {reward_tx.get('block_height')} vs {block_index}")
            return False
    ...
    def mine_and_submit(self):
        ...
        try:
            ...
            total_fees = sum(tx.get('fee', 0) for tx in mempool_txs if tx.get('type') == 'transfer')
            ...
            if not self.validate_reward_transaction(reward_tx, next_index):
                logger.error("❌ Reward transaction validation failed, cannot mine block")
                return False
            
            # Combine transactions and validate
            all_transactions = [reward_tx] + mempool_txs
            valid_transactions = self.validate_transactions_for_block(all_transactions, next_index)
            
            logger.info(f"📄 Block transactions: {len(valid_transactions)} total ({len(mempool_txs)} mempool + 1 reward)")
            
            # Create and mine block with CORRECT previous hash
            block = RealBlock(next_index, previous_hash, valid_transactions, miner=self.miner_address)
            ...
            if block.mine_block(difficulty):
                ...
                if verify_hash != block.hash:
                    logger.error(f"❌ Hash verification failed: {block.hash} vs {verify_hash}")
                    return False
    ...
    def print_stats(self):
        ...
        with self.stats_lock:
            ...
            logger.info(f"   Transfers submitted: {self.transfers_submitted}")
            logger.info(f"   Total rewards: {self.rewards_earned:.2f} LUN")
            logger.info(f"   Active transfer workers: {len([t for t in self.transfer_threads if t.is_alive()])}")
...
def debug_server_state():
    ...
    try:
        ...
        if status_response.status_code == 200:
            status_data = status_response.json()
            status_info = status_data.get('status', {})
            logger.info(f"📊 STATUS: blocks={status_info.get('blocks')}, latest_hash={status_info.get('latest_hash', '')[:16]}...")
        ...
        if blockchain_response.status_code == 200:
            ...
            for i, block in enumerate(blockchain_data):
                logger.info(f"   Block {i}: index={block.get('index')}, hash={block.get('hash', '')[:16]}...")
...
def main():
    ...
    miner_address = "threaded_miner_" + hashlib.sha256(str(time.time()).encode()).hexdigest()[:8]
