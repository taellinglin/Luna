    ...
    def handle_wallet_connection(self, data: Dict):
        ...
        try:
            if action == "ping":
                ...
                estimated_size = len(
                    json.dumps([block.__dict__ for block in self.blockchain.chain])
                )
                ...
                max_blocks = 10
                chain_to_send = (
                    self.blockchain.chain[-max_blocks:]
                    if len(self.blockchain.chain) > max_blocks
                    else self.blockchain.chain
                )
                ...
                return {
                    "status": "success",
                    "difficulty": self.blockchain.difficulty,
                    "available_bills": self.blockchain.difficulty_denominations.get(
                ...
                return {
                    "status": "success",
                    "message": f"Loaded {len(txs)} transactions",
                }
