...
from luna_node import Blockchain, configure_ssl_for_frozen_app
...
class ConsoleOutput(ScrollView):
    ...
    def __init__(self, **kwargs):
        ...
        self.layout = GridLayout(
            cols=1, size_hint_y=None, spacing=1, padding=2
        )  # Tighter spacing
...
class ControlPanel(GridLayout):
    ...
    def create_controls(self):
        ...
        # Difficulty Controls
        diff_label = DarkLabel(
            text="Difficulty:", size_hint_x=None, width=80, height=20
        )
        ...
        self.peer_address = DarkTextInput(
            text="", hint_text="ip:port", size_hint_x=None, width=150, height=20
        )
        ...
        discover_btn = DarkButton(
            text="Discover", size_hint_x=None, width=60, height=20
        )
        ...
        def mining_thread():
            try:
                ...
                if success:
                    ...
                    self.main_app.log(
                        "Mining failed or no transactions to mine", "warning"
                    )
            ...
            try:
                ...
                for tx in txs:
                    if self.main_app.blockchain.add_transaction(tx):
                        loaded_count += 1
                self.main_app.log(
                    f"Added {loaded_count}/{len(txs)} transactions", "success"
                )
            except Exception as e:
    ...
    def set_difficulty(self, instance):
        try:
            ...
            if 1 <= new_diff <= 8:
                ...
                bills = self.main_app.blockchain.difficulty_denominations.get(
                    new_diff, {}
                )
    ...
    def show_status(self, instance):
        ...
        self.main_app.log(
            f"  Height: {len(self.main_app.blockchain.chain)} blocks", "info"
        )
        self.main_app.log(
            f"  Pending TXs: {len(self.main_app.blockchain.pending_transactions)}",
            "info",
        )
        self.main_app.log(f"  Latest Block: {latest.hash[:16]}...", "info")
        self.main_app.log(f"  Total Blocks Mined: {stats['total_blocks']}", "info")
        self.main_app.log(
            f"  Known Peers: {len(self.main_app.blockchain.peers)}", "info"
        )
    ...
    def show_stats(self, instance):
        ...
        self.main_app.log(
            f"  Difficulty: {self.main_app.blockchain.difficulty}", "info"
        )
...
class StatusBar(BoxLayout):
    ...
    def __init__(self, main_app, **kwargs):
        ...
        self.node_label = DarkLabel(
            text="Node...", size_hint_x=None, width=100, height=20
        )
...
class LunaNodeGUI(TabbedPanel):
    ...
    def log(self, message, message_type="info"):
        ...
        Clock.schedule_once(
            lambda dt: self.console_output.add_message(message, message_type), 0
        )
