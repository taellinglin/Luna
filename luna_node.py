...
class NodeConfig:
    ...
    def load_config(self):
        ...
        if not isinstance(config.get("mined_transactions"), dict):
            print(
                color_text(
                    "⚠️  Fixing mined_transactions type from list to dict", COLORS["O"]
                )
            )
        ...
        self.mined_transactions = config[
            "mined_transactions"
        ]  # This is now guaranteed to be a dict
    ...
    def validate_config(self):
        ...
        if not 1 <= self.difficulty <= 8:
            self.difficulty = 4
            print(color_text("⚠️  Reset difficulty to default (4)", COLORS["O"]))
        
        if self.mining_reward < 0:
            self.mining_reward = 1.0
            print(color_text("⚠️  Reset mining reward to default (1.0)", COLORS["O"]))
        
        if not self.miner_address:
            self.miner_address = self.generate_miner_address()
            print(color_text("⚠️  Generated new miner address", COLORS["O"]))
    ...
    def add_bill(
        self, block_hash, amount, timestamp, verification_url="", transaction_count=0
    ):
    ...
    def get_bills_summary(self):
        ...
        total_transactions = sum(
            bill.get("transactions_mined", 0) for bill in self.bills_mined
        )
    ...
    def mark_transaction_mined(self, tx_signature, block_hash):
        ...
        print(color_text("🔍 [DEBUG] START: mark_transaction_mined", COLORS["B"]))
        print(
            color_text(f"🔍 [DEBUG] tx_signature: {tx_signature[:20]}...", COLORS["B"])
        )
        print(color_text(f"🔍 [DEBUG] block_hash: {block_hash[:20]}...", COLORS["B"]))
        print(
            color_text(
                f"🔍 [DEBUG] mined_transactions type: {type(self.mined_transactions)}",
                COLORS["B"],
            )
        )
        ...
        if not isinstance(self.mined_transactions, dict):
            print(
                color_text(
                    f"⚠️  mined_transactions is {type(self.mined_transactions)}, converting to dict",
                    COLORS["O"],
                )
            )
            print(
                color_text(
                    f"🔍 [DEBUG] Current mined_transactions value: {self.mined_transactions}",
                    COLORS["O"],
                )
            )
            ...
            if isinstance(self.mined_transactions, list):
                ...
                print(
                    color_text(
                        f"🔍 [DEBUG] Converted list to dict with {len(new_mined_txs)} entries",
                        COLORS["G"],
                    )
                )
            else:
                ...
                print(
                    color_text(
                        "🔍 [DEBUG] Initialized empty dict for mined_transactions",
                        COLORS["G"],
                    )
                )
        ...
        print(
            color_text(
                f"🔍 [DEBUG] Adding to mined_transactions (current size: {len(self.mined_transactions)})",
                COLORS["B"],
            )
        )
        ...
        print(
            color_text(
                f"🔍 [DEBUG] Added successfully (new size: {len(self.mined_transactions)})",
                COLORS["G"],
            )
        )
        ...
        print(
            color_text("🔍 [DEBUG] END: mark_transaction_mined - SUCCESS", COLORS["G"])
        )
    ...
    def is_transaction_mined(self, tx_signature):
        ...
        if not isinstance(self.mined_transactions, dict):
            print(
                color_text(
                    f"⚠️  mined_transactions is {type(self.mined_transactions)} in is_transaction_mined, returning False",
                    COLORS["O"],
                )
            )
            return False
...
class Block:
    ...
    def from_dict(block_dict):
        ...
        if not isinstance(block_dict, dict):
            print("❌ Block data is not a dictionary")
            return Block(0, "0", [{"type": "error", "message": "Invalid block data"}])
        ...
        try:
            ...
            if not isinstance(previous_hash, str):
                print(
                    f"⚠️  Invalid previous_hash type: {type(previous_hash)}, converting to str"
                )
            ...
            if not isinstance(transactions, list):
                print(
                    f"⚠️  Invalid transactions type: {type(transactions)}, using empty list"
                )
                transactions = []
            ...
            if "mining_time" in block_dict and isinstance(
                block_dict["mining_time"], (int, float)
            ):
            ...
            print(
                f"📋 Problematic block data: {json.dumps(block_dict, indent=2)[:500]}..."
            )
            # Return a default block to prevent crashes
            return Block(
                0, "0", [{"type": "error", "message": f"Failed to load block: {e}"}]
            )
    ...
    def mine_block(self, difficulty, progress_callback=None):
        ...
        while self.hash[:difficulty] != target:
            ...
            if current_time - last_update >= 0.5:
                ...
                progress_bar = (
                    f"{color}[{'█' * filled}{'░' * (30 - filled)}]{COLORS['X']}"
                )

                hash_rate_str = (
                    f"{avg_hash_rate / 1000:.1f} kH/s"
                    if avg_hash_rate > 1000
                    else f"{avg_hash_rate:.1f} H/s"
                )

                print(
                    f"\r{color}⛏️  {progress_bar} {progress:.1%} | {hash_rate_str} | Nonce: {self.nonce:,}{COLORS['X']}",
                    end="",
                )
        ...
        print(
            color_text(
                f"\n✅ Block mined in {self.mining_time:.2f}s | Hash: {self.hash[:16]}...",
                COLORS["G"],
            )
        )
        return True
...
class Blockchain:
            ...
            for block in blocks:
                if isinstance(block, Block):
                    ...
                    try:
                        ...
                        print(
                            color_text(f"❌ Error converting block: {e}", COLORS["R"])
                        )
                        continue
        ...
        try:
            ...
            if hasattr(self, "mempool") and self.mempool is not None:
                if isinstance(self.mempool, list) and len(self.mempool) > 0:
                    print(
                        color_text(
                            f"📋 Using blockchain mempool ({len(self.mempool)} transactions)",
                            COLORS["G"],
                        )
                    )
                    return self.mempool
            ...
            if isinstance(mempool_data, list) and len(mempool_data) > 0:
                print(
                    color_text(
                        f"📋 Loaded {len(mempool_data)} transactions from local mempool file",
                        COLORS["G"],
                    )
                )
                return mempool_data
                ...
                print(
                    color_text("📭 Local mempool file is empty or invalid", COLORS["O"])
                )
                return []
    ...
    def save_mempool(self):
        ...
        try:
            ...
            print(
                color_text(
                    f"💾 Saved {len(mempool_to_save)} transactions to local mempool",
                    COLORS["G"],
                )
            )
            return True
    ...
    def update_mempool(self, new_mempool):
        ...
        try:
            ...
            print(
                color_text(
                    f"🔄 Updated mempool to {len(new_mempool)} transactions",
                    COLORS["G"],
                )
            )
            return True
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(api_mempool, list):
                    print(
                        color_text(
                            f"📥 Downloaded {len(api_mempool)} transactions from API",
                            COLORS["G"],
                        )
                    )
                    ...
                    if new_transactions > 0:
                        ...
                        genesis_txs = [
                            tx
                            for tx in merged_mempool
                            if tx.get("type") == "GTX_Genesis"
                        ]
                        if genesis_txs:
                            print(
                                color_text(
                                    f"💰 Found {len(genesis_txs)} Genesis transactions!",
                                    COLORS["Y"],
                                )
                            )
                        ...
                        print(
                            color_text("ℹ️  Mempool is already up to date", COLORS["B"])
                        )
                        return True
                else:
                ...
                print(
                    color_text(
                        f"❌ API returned status {response.status_code}", COLORS["R"]
                    )
                )
    ...
    def _process_block_list(self, block_list):
        ...
        for i, block_data in enumerate(block_list):
            try:
                if isinstance(block_data, (dict, Block)):
                    ...
                    print(
                        color_text(
                            f"⚠️  Invalid block type at index {i}: {type(block_data)}",
                            COLORS["O"],
                        )
                    )
            except Exception as e:
        ...
        if loaded_chain:
            print(
                color_text(
                    f"✅ Loaded {len(loaded_chain)} blocks from storage", COLORS["G"]
                )
            )
            return loaded_chain
    ...
    def _get_block_info(self, block):
        ...
        if isinstance(block, Block):
            ...
            return {"index": 0, "hash": "unknown", "transactions": []}
    ...
    def save_chain(self):
        ...
        try:
            ...
            if os.path.exists(os.path.join(ConfigManager.get_data_dir(), self.chain_file)):
                backup_file = os.path.join(ConfigManager.get_data_dir(), f"{self.chain_file}.backup")
                shutil.copy2(
                    os.path.join(ConfigManager.get_data_dir(), self.chain_file),
                    backup_file
                )
            ...
            for block in self.chain:
                if isinstance(block, Block):
                    ...
                    print(
                        color_text(
                            f"⚠️  Skipping invalid block type: {type(block)}", COLORS["O"]
                        )
                    )

            ConfigManager.save_json(self.chain_file, chain_data)
            print(color_text("💾 Blockchain saved successfully", COLORS["G"]))
        except Exception as e:
            print(color_text(f"❌ Error saving blockchain: {e}", COLORS["R"]))
    ...
    def auto_sync(self):
        ...
        if self.sync_from_web():
            ...
            print(
                color_text("⚠️  Blockchain sync failed, using local chain", COLORS["O"])
            )
        ...
        if self.sync_mempool_from_api():
            ...
            print(
                color_text("⚠️  Mempool sync failed, using local mempool", COLORS["O"])
            )
        ...
        for api_tx in api_mempool:
            ...
            if api_signature not in local_signatures:
                ...
                local_signatures.add(
                    api_signature
                )  # Add to set to avoid duplicates in this loop

        print(
            color_text(
                f"📊 Mempool merge: {len(local_mempool)} local + {added_count} new = {len(merged_mempool)} total",
                COLORS["G"],
            )
        )
    ...
    def sync_from_web(self):
        ...
        try:
            ...
            response = requests.get(
                f"{self.config.api_base_url}/blockchain", timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                if isinstance(remote_chain_data, list):
                    ...
                    for block_dict in remote_chain_data:
                        if isinstance(block_dict, dict):
                            try:
                                ...
                                print(
                                    color_text(
                                        f"⚠️  Error converting block: {e}", COLORS["O"]
                                    )
                                )
                                continue
                    ...
                    if remote_chain and len(remote_chain) > len(self.chain):
                        print(
                            color_text(
                                f"🔄 Downloading {len(remote_chain) - len(self.chain)} new blocks",
                                COLORS["G"],
                            )
                        )
                    ...
                    print(
                        color_text("❌ Invalid response format from API", COLORS["R"])
                    )
            else:
                print(
                    color_text(
    ...
    def get_blockchain_status(self):
        ...
        try:
            response = requests.get(
                f"{self.config.api_base_url}/api/blockchain/status", timeout=10
            )
            if response.status_code == 200:
    ...
    def get_block_details(self, block_hash):
        ...
        try:
            response = requests.get(
                f"{self.config.api_base_url}/api/block/{block_hash}", timeout=10
            )
            if response.status_code == 200:
                result = response.json()
                if result.get("status") == "success":
    ...
    def _ensure_valid_chain_structure(self):
        ...
        if not isinstance(self.chain, list):
            print(
                color_text(
                    "⚠️  Blockchain is not a list, initializing new chain", COLORS["O"]
                )
            )
        ...
        if len(self.chain) == 0:
            print(
                color_text("⚠️  Blockchain is empty, adding genesis block", COLORS["O"])
            )
            self.chain = [self.create_genesis_block()]
            return
        ...
        if isinstance(self.chain, dict):
            print(
                color_text("⚠️  Chain is a dictionary, converting to list", COLORS["O"])
            )
            new_chain = []
            ...
            for key, value in self.chain.items():
                if isinstance(value, (dict, Block)):
                    ...
                    if isinstance(value, dict) and (
                        "index" in value or "transactions" in value or "hash" in value
                    ):
                        try:
                            ...
                            print(
                                color_text(
                                    f"⚠️  Could not convert dict to block: {e}",
                                    COLORS["O"],
                                )
                            )
                    else:
            ...
            if not new_chain:
                print(
                    color_text(
                        "⚠️  No valid blocks found in dictionary, creating new chain",
                        COLORS["O"],
                    )
                )
    ...
    def add_transaction(self, transaction_data):
        ...
        try:
            ...
            print(
                color_text(
                    f"📋 Created transaction: {transaction['hash']}", COLORS["B"]
                )
            )
            ...
            print(
                color_text(
                    f"🌐 Broadcasting to {len(broadcast_targets)} targets", COLORS["B"]
                )
            )
            ...
            if success_count > 0:
                print(
                    color_text(
                        f"✅ Successfully broadcasted to {success_count}/{len(broadcast_targets)} targets",
                        COLORS["G"],
                    )
                )
                return True
            else:
                print(color_text("❌ Failed to broadcast to any targets", COLORS["R"]))
                return False
    ...
    def get_broadcast_targets(self):
        ...
        # Add RPC nodes
        targets.extend(self.peers)

        # Add API endpoints
        api_endpoints = ["https://bank.linglin.art/api/transaction/broadcast"]

        # Add base_url endpoint if available
        if hasattr(self, "base_url") and self.base_url:
            api_endpoints.append(
                f"{self.base_url.rstrip('/')}/api/transaction/broadcast"
            )
    ...
    def broadcast_reward_transaction(self, reward_data):
        ...
        try:
            print(
                color_text("🌐 Broadcasting reward transaction to API...", COLORS["B"])
            )
            ...
            reward_tx = {
                ...
                "block_height": int(
                    reward_data.get("block_height", 0)
                ),  # Ensure it's an int
                "type": "reward",
            }
            ...
            if not reward_tx["to"]:
                print(
                    color_text(
                        "❌ Cannot broadcast reward: missing 'to' address", COLORS["R"]
                    )
                )
                return False
            ...
            if reward_tx["amount"] <= 0:
                print(
                    color_text(
                        "❌ Cannot broadcast reward: invalid amount", COLORS["R"]
                    )
                )
                return False
            ...
            if response.status_code == 200:
                ...
                if result.get("status") == "success":
                    print(
                        color_text(
                            "✅ Reward transaction broadcasted successfully!",
                            COLORS["G"],
                        )
                    )
                    print(
                        color_text(
                            f"📝 Transaction hash: {result.get('transaction_hash', 'unknown')}",
                            COLORS["B"],
                        )
                    )
                    return True
            ...
            print(
                color_text(
                    f"❌ Error broadcasting reward transaction: {e}", COLORS["R"]
                )
            )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("status") == "success":
                        print(
                            color_text(
                                f"✅ Accepted by {self.get_target_name(target)}",
                                COLORS["G"],
                            )
                        )
                        success_count += 1
                        ...
                        print(
                            color_text(
                                f"❌ {self.get_target_name(target)} rejected: {result.get('message')}",
                                COLORS["R"],
                            )
                        )
                else:
                    print(
                        color_text(
                            f"⚠️  {self.get_target_name(target)} returned {response.status_code}",
                            COLORS["O"],
                        )
                    )
                ...
                print(
                    color_text(
                        f"❌ Failed to reach {self.get_target_name(target)}: {str(e)}",
                        COLORS["R"],
                    )
                )
            except Exception as e:
                print(
                    color_text(
                        f"❌ Error broadcasting to {self.get_target_name(target)}: {str(e)}",
                        COLORS["R"],
                    )
                )
...
class LunaNode:
    def __init__(self):
        ...
        print(
            color_text(
                f"\n🌙 {COLORS['BOLD']}Luna Node v{self.config.node_version}{COLORS['X']}",
                COLORS["I"],
            )
        )
        ...
        print(
            color_text(
                f"🔗 Chain Length: {self.blockchain.get_chain_length()}", COLORS["Y"]
            )
        )
        print(
            color_text(f"📊 Mempool Size: {len(self.blockchain.mempool)}", COLORS["O"])
        )
    ...
    def check_network_status(self):
        ...
        try:
            ...
            network_status = "✅ Connected" if response.status_code == 200 else "❌ Disconnected"
        except:
            network_status = "❌ Disconnected"
        
        status_color = COLORS["G"] if "✅" in network_status else COLORS["R"]
        print(color_text(f"🌐 Network: {network_status}", status_color))
    ...
    def start_auto_mining(self):
        ...
        self.auto_mine_thread = threading.Thread(
            target=self._auto_mine_worker, daemon=True
        )
        ...
        while self.running and self.config.auto_mine:
            try:
                ...
                if unmined_txs:
                    print(
                        color_text(
                            f"🤖 Auto-mining {len(unmined_txs)} unmined transactions...",
                            COLORS["G"],
                        )
                    )
                    self.mine_block()
                    ...
                    if self.blockchain.mempool:
                        print(
                            color_text(
                                f"🤖 Auto-mining {len(self.blockchain.mempool)} mempool transactions...",
                                COLORS["G"],
                            )
                        )
    ...
    def mine_from_mempool(self):
        ...
        # Use thread lock to prevent concurrent mining
        if self._mining_lock.locked():
            print(color_text("⚠️  Mining already in progress", COLORS["O"]))
            return False
        ...
        with self._mining_lock:
            ...
            try:
                ...
                print(
                    color_text(f"📊 Mempool has {len(mempool)} transactions", COLORS["B"])
                )
                ...
                for tx in mempool:
                    ...
                    if not self.config.is_transaction_mined(tx_signature):
                        ...
                        print(
                            color_text(
                                f"⏭️  Skipping already mined transaction: {tx_signature[:16]}...",
                                COLORS["O"],
                            )
                        )
                ...
                print(
                    color_text(
                        f"⛏️  Preparing to mine {len(unmined_txs)} unmined transactions from mempool",
                        COLORS["G"],
                    )
                )
                ...
                new_index = (
                    ...
                    if hasattr(latest_block, "index")
                    else len(self.blockchain.chain)
                )
                ...
                def mining_progress(hashes_tried, hash_rate, progress):
                    if hashes_tried % 1000 == 0:
                        hash_rate_str = (
                            f"{hash_rate / 1000:.1f} kH/s"
                            if hash_rate > 1000
                            else f"{hash_rate:.1f} H/s"
                        )
                        print(
                            f"⛏️  Mining... {hashes_tried:,} hashes | {hash_rate_str} | {progress:.1%}"
                        )

                print(
                    color_text(
                        f"🎯 Starting to mine block #{new_index} with difficulty {self.config.difficulty}",
                        COLORS["B"],
                    )
                )
                ...
                if new_block.mine_block(self.config.difficulty, mining_progress):
                    ...
                    if self.blockchain.broadcast_reward_transaction(reward_data):
                        print(
                            color_text(
                                "💰 Reward successfully submitted to network!", COLORS["G"]
                            )
                        )
                    else:
                        print(
                            color_text(
                                "⚠️  Reward submission failed, but block was mined locally",
                                COLORS["O"],
                            )
                        )
                    ...
                    print(
                        color_text(
                            f"✅ Block #{new_index} mined successfully from mempool!",
                            COLORS["G"],
                        )
                    )
    ...
    def _remove_mined_from_mempool(self, mined_transactions):
        ...
        try:
            ...
            mined_signatures = {
                self.blockchain.get_transaction_signature(tx)
            ...
            if removed_count > 0:
                ...
                print(
                    color_text(
                        f"🗑️  Removed {removed_count} mined transactions from mempool",
                        COLORS["G"],
                    )
                )
            else:
                print(
                    color_text("ℹ️  No transactions to remove from mempool", COLORS["B"])
                )

        except Exception as e:
            print(
                color_text(
                    f"❌ Error removing transactions from mempool: {e}", COLORS["R"]
                )
            )
    ...
    def mine_block(self):
        ...
        with self._mining_lock:
            ...
            try:
                ...
                print(
                    color_text(
                        f"⛏️  Found {len(unmined_txs)} unmined transactions", COLORS["G"]
                    )
                )
                ...
                reward_tx = {
                    ...
                    + len(transactions) * 0.1,  # Base + transaction fees
                    "timestamp": time.time(),
                ...
                def mining_progress(hashes_tried, hash_rate, progress):
                    if hashes_tried % 1000 == 0:
                        print(
                            f"⛏️  Mining... {hashes_tried:,} hashes | {hash_rate:.1f} H/s | {progress:.1%}"
                        )
                ...
                if new_block.mine_block(self.config.difficulty, mining_progress):
                    ...
                    for tx_info in unmined_txs:
                        self.config.mark_transaction_mined(
                            tx_info["signature"], new_block.hash
                        )
                    ...
                    print(
                        color_text(
                            f"✅ Block #{new_index} mined successfully!", COLORS["G"]
                        )
                    )
                    print(color_text(f"💰 Reward: {total_reward} LUN", COLORS["Y"]))
                    print(color_text(f"📊 Transactions: {len(unmined_txs)}", COLORS["B"]))
    ...
    def show_status(self):
        ...
        print(
            color_text(
                f"💰 Mining Reward: {self.config.mining_reward} LUN", COLORS["I"]
            )
        )
        print(
            color_text(
                f"🤖 Auto-mine: {'Enabled' if self.config.auto_mine else 'Disabled'}",
                COLORS["V"],
            )
        )
        ...
        if self.config.last_sync > 0:
            last_sync_time = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(self.config.last_sync)
            )
            print(color_text(f"🔄 Last Sync: {last_sync_time}", COLORS["I"]))

        if self.config.last_mempool_sync > 0:
            last_mempool_sync = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(self.config.last_mempool_sync)
            )
    ...
    def show_menu(self):
        ...
        while self.running:
            ...
            try:
                choice = input(
                    color_text("\n🎯 Enter your choice (1-8): ", COLORS["BOLD"])
                ).strip()
    ...
    def settings_menu(self):
        ...
        while True:
            ...
            print("=" * 40)
            print(
                color_text(f"1. 🎯 Difficulty: {self.config.difficulty}", COLORS["B"])
            )
            print(
                color_text(
                    f"2. 💰 Mining Reward: {self.config.mining_reward} LUN", COLORS["G"]
                )
            )
            print(
                color_text(
                    f"3. ⛏️  Miner Address: {self.config.miner_address}", COLORS["Y"]
                )
            )
            ...
            try:
                ...
                if choice == "1":
                    new_diff = input(
                        f"Enter new difficulty (1-8, current: {self.config.difficulty}): "
                    ).strip()
                    try:
                        ...
                        if self.config.update_difficulty(new_diff):
                            print(
                                color_text(
                                    f"✅ Difficulty updated to {new_diff}", COLORS["G"]
                                )
                            )
                        else:
                            print(
                                color_text(
                                    "❌ Invalid difficulty (must be 1-8)", COLORS["R"]
                                )
                            )
                    except ValueError:
                    ...
                    new_reward = input(
                        f"Enter new mining reward (current: {self.config.mining_reward}): "
                    ).strip()
                    try:
                        ...
                        if new_reward > 0:
                            ...
                            print(
                                color_text(
                                    f"✅ Mining reward updated to {new_reward}",
                                    COLORS["G"],
                                )
                            )
                        else:
                    ...
                    new_address = input(
                        f"Enter new miner address (current: {self.config.miner_address}): "
                    ).strip()
                    if new_address:
                        self.config.miner_address = new_address
                        self.config.save_config()
                        print(color_text("✅ Miner address updated", COLORS["G"]))
                    else:
    ...
    def show_mining_history(self):
        ...
        for i, bill in enumerate(reversed(self.config.bills_mined[-10:]), 1):
            ...
            print(
                color_text(
                    f"   Transactions: {bill.get('transactions_mined', 0)}", COLORS["O"]
                )
            )
