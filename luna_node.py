...
class NodeConfig:
    ...
    def __init__(self):
        ...
        if (
            not hasattr(self, "miner_address")
            or not self.miner_address
            or self.miner_address == "LUN_e93d86a530870259_7fdd8f27"
        ):
            self.setup_new_node()
    ...
    def setup_new_node(self):
        ...
        print("=" * 50)
        print(
            color_text("Welcome to Luna Node! Let's configure your node.", Colors.BLUE)
        )
        print()
        ...
        while True:
            address = input(
                color_text("💰 Enter your wallet address: ", Colors.YELLOW)
            ).strip()
            if address:
        ...
        while True:
            try:
                diff = input(
                    color_text(
                        "🎯 Enter mining difficulty (1-8, recommended 4): ",
                        Colors.YELLOW,
                    )
                ).strip()
                if not diff:
        ...
        auto_mine = (
            input(color_text("🤖 Enable auto-mining? (y/N): ", Colors.YELLOW))
            .strip()
            .lower()
        )
        ...
        if CUDA_AVAILABLE:
            use_cuda = (
                input(color_text("🎮 Use CUDA for mining? (Y/n): ", Colors.YELLOW))
                .strip()
                .lower()
            )
        ...
        print(
            color_text(
                f"🤖 Auto-mine: {'Enabled' if self.auto_mine else 'Disabled'}",
                Colors.MAGENTA,
            )
        )
        print(
            color_text(
                f"🎮 CUDA: {'Enabled' if self.use_cuda else 'Disabled'}", Colors.CYAN
            )
        )
        print()
    ...
    def update_wallet_address(self):
        ...
        while True:
            new_address = input(
                color_text("Enter new wallet address: ", Colors.YELLOW)
            ).strip()
            if new_address:
    ...
    def update_difficulty(self):
        ...
        print(
            color_text(
                "Note: Higher difficulty requires more computation but finds blocks less frequently.",
                Colors.YELLOW,
            )
        )
        ...
        while True:
            try:
                new_diff = input(
                    color_text("Enter new difficulty (1-8): ", Colors.YELLOW)
                ).strip()
                if not new_diff:
                    continue
                ...
                if 1 <= new_diff <= 8:
                    ...
                    print(
                        color_text(
                            f"✅ Difficulty updated from {old_diff} to {new_diff}",
                            Colors.GREEN,
                        )
                    )
                    break
    ...
    def get_rewards_summary(self):
        ...
        total_amount = sum(
            reward["amount"] for reward in self.reward_transactions_mined
        )
...
class RealBlock:
    ...
    def mine_block(self, difficulty):
        ...
        print(
            color_text(
                f"⛏️ Mining Block #{self.index} | Target: {target}...", Colors.CYAN
            )
        )
        print(
            color_text(f"   Previous hash: {self.previous_hash[:16]}...", Colors.BLUE)
        )
        ...
        while self.hash[:difficulty] != target:
            ...
            if attempts % 100000 == 0:
                ...
                print(
                    color_text(
                        f"   Attempts: {attempts:,} | Rate: {hash_rate:,.0f} H/s | Hash: {self.hash[:16]}...",
                        Colors.YELLOW,
                    )
                )
            ...
            if attempts > 2000000:
                print(
                    color_text(
                        "🔄 Restarting mining with new nonce range...", Colors.YELLOW
                    )
                )
                self.nonce = random.randint(0, 1000000)
                attempts = 0
        ...
        print(
            color_text(f"✅ Block mined! Time: {self.mining_time:.2f}s", Colors.GREEN)
        )
...
class SmartMiner:
    ...
    def _refresh_reward_cache(self):
        ...
        try:
            ...
            if blockchain_response.status_code == 200:
                ...
                recent_blocks = (
                    blockchain_data[-50:]
                    if len(blockchain_data) > 50
                    else blockchain_data
                )
                ...
                print(
                    color_text(
                        f"✅ Reward cache updated: {len(self._reward_cache)} mined rewards",
                        Colors.GREEN,
                    )
                )
    ...
    def get_mempool_transactions(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    color_text(
                        f"📥 Transactions loaded in {processing_time:.2f}s: {len(valid_txs)} regular + {len(unmined_rewards)} unmined rewards",
                        Colors.BLUE,
                    )
                )
    ...
    def _filter_unmined_rewards_parallel(self, reward_txs):
        ...
        if skipped_count > 0:
            print(
                color_text(
                    f"⏭️  Skipped {skipped_count} already mined rewards", Colors.YELLOW
                )
            )
    ...
    def scan_for_unmined_rewards(self, current_block_index):
        ...
        try:
            ...
            if blockchain_response.status_code == 200:
                    ...
                    for tx in transactions:
                        if tx.get("type") == "reward":
                            ...
                            if tx.get("block_height") == current_block_index:
                                ...
                                print(
                                    color_text(
                                        f"⏭️  Skipping reward from wrong block {tx.get('block_height')} (mining block {current_block_index})",
                                        Colors.ORANGE,
                                    )
                                )
                ...
                print(
                    color_text(
                        f"💰 Found {len(unmined_rewards)} unmined reward transactions for block #{current_block_index}",
                        Colors.GREEN,
                    )
                )
                return unmined_rewards
        ...
        try:
            ...
            if not already_marked:
                ...
                if len(self.config.reward_transactions_mined) > 200:
                    self.config.reward_transactions_mined = (
                        self.config.reward_transactions_mined[-200:]
                    )
        ...
        try:
            ...
            if block.mine_block(difficulty):
                ...
                if not self.config.auto_mine:
                    confirm = (
                        input(color_text("Submit this block? (y/N): ", Colors.BOLD))
                        .strip()
                        .lower()
                    )
                ...
                if self.submit_block(block_data):
                    ...
                    for tx in all_external_txs:
                        if tx.get("type") == "reward":
                            ...
                            print(
                                color_text(
                                    f"✅ Marked reward TX as mined: {tx.get('to')} for {tx.get('amount')} LUN",
                                    Colors.GREEN,
                                )
                            )
                    ...
                    if reward_txs_marked > 0:
                        print(
                            color_text(
                                f"🎯 Marked {reward_txs_marked} reward transactions as mined",
                                Colors.MAGENTA,
                            )
                        )
                    ...
                    print(
                        color_text(
                            f"🎯 Our Reward TX: {our_reward_tx['hash'][:16]}...",
                            Colors.MAGENTA,
                        )
                    )
                    print(
                        color_text(f"📝 Block Hash: {block.hash[:16]}...", Colors.CYAN)
                    )
                    return True
                else:
                    print(
            ...
            if response.status_code == 201:
                ...
                if result.get("success"):
                    ...
                    if self.wait_for_block_confirmation(block_data["hash"]):
                        print(
                            color_text("🎉 Block confirmed on network!", Colors.GREEN)
                        )
                        return True
                    else:
                        print(
                            color_text(
                                "⚠️ Block submitted but not yet confirmed", Colors.ORANGE
                            )
                        )
        ...
        try:
            ...
            if latest_block and hasattr(latest_block, "index"):
                ...
                if block_data["index"] != expected_index:
                    print(
                        color_text(
                            f"❌ Block index mismatch. Expected: {expected_index}, Got: {block_data['index']}",
                            Colors.RED,
                        )
                    )
                    return False
    ...
    def verify_block_hash(self, block_data):
        ...
        try:
            ...
            if calculated_hash != block_data["hash"]:
                print(
                    color_text(
                        f"❌ Hash mismatch: {calculated_hash} vs {block_data['hash']}",
                        Colors.RED,
                    )
                )
                return False
        ...
        for attempt in range(max_attempts):
            try:
                ...
                response = requests.get(
                    f"{self.config.api_base_url}/blockchain", timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(blockchain_data, list):
                        ...
                        for block in blockchain_data:
                            if block.get("hash") == block_hash:
                                print(
                                    color_text(
                                        f"✅ Block found at position {block.get('index')}",
                                        Colors.GREEN,
                                    )
                                )
                                return True
                    ...
                    print(
                        color_text(
                            f"🔍 Confirmation attempt {attempt + 1}/{max_attempts}...",
                            Colors.BLUE,
                        )
                    )
...
class LunaNode:
    ...
    def show_welcome(self):
        ...
        print(
            color_text(f"💰 Base Reward: {self.config.mining_reward} LUN", Colors.BLUE)
        )
        print(
            color_text(
                f"💸 Transaction Fee: {self.config.transaction_fee} LUN", Colors.MAGENTA
            )
        )
        print(
            color_text(
    ...
    def auto_sync(self):
        ...
        try:
            ...
            print(
                color_text(
                    f"📊 Network height: {block_info['reported_height']}", Colors.GREEN
                )
            )
            print(
                color_text(
                    f"🎯 Network difficulty: {block_info['difficulty']}", Colors.YELLOW
                )
            )
            print(
                color_text(
                    f"🔗 Last block hash: {block_info['previous_hash'][:16]}...",
                    Colors.BLUE,
                )
            )
    ...
    def toggle_auto_mine(self):
        ...
        if self.miner.is_mining:
            ...
            self.auto_mine_thread = threading.Thread(
                target=self.miner.start_auto_mining, daemon=True
            )
    ...
    def mine_once(self):
        ...
        if self.miner.is_mining:
            print(
                color_text(
                    "⚠️  Auto-mining is running. Stop it first or wait for next block.",
                    Colors.YELLOW,
                )
            )
            return
    ...
    def show_status(self):
        ...
        try:
            ...
            print(
                color_text(
                    f"🌐 Network Height: {block_info['reported_height']}", Colors.BLUE
                )
            )
            print(
                color_text(
            ...
            print(
                color_text(
                    f"🔗 Previous Hash: {block_info['previous_hash'][:16]}...",
                    Colors.BLUE,
                )
            )
        except:
        ...
        print(
            color_text(
                f"🎯 Configured Difficulty: {self.config.difficulty}", Colors.YELLOW
            )
        )
        print(color_text(f"📈 Blocks Mined: {self.miner.blocks_mined}", Colors.GREEN))
        print(
            color_text(
                f"🤖 Auto-mine: {'Running' if self.miner.is_mining else 'Stopped'}",
                Colors.MAGENTA,
            )
        )
    ...
    def show_bills(self):
        ...
        print(
            color_text(
                f"Total Bills: {total_bills} | Total Reward: {total_reward} LUN | Total Transactions: {total_txs}",
                Colors.GREEN,
            )
        )
        print()
        ...
        for i, bill in enumerate(reversed(self.config.bills_mined[-10:]), 1):
            ...
            print(
                color_text(
                    f"   Transactions: {bill.get('transactions_mined', 0)}",
                    Colors.GREEN,
                )
            )
            if bill.get("reward_tx_hash"):
                print(
                    color_text(
                        f"   Reward TX: {bill['reward_tx_hash'][:16]}...",
                        Colors.MAGENTA,
                    )
                )
            print()
    ...
    def show_rewards(self):
        ...
        print(
            color_text(
                f"Total Rewards: {total_rewards} | Total Amount: {total_amount} LUN",
                Colors.GREEN,
            )
        )
        print()
        ...
        for i, reward in enumerate(
            reversed(self.config.reward_transactions_mined[-10:]), 1
        ):
    ...
    def show_menu(self):
        ...
        while self.running:
            ...
            try:
                choice = input(
                    color_text("\n🎯 Enter your choice (1-9): ", Colors.BOLD)
                ).strip()
